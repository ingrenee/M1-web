...
class Lib_formacion
{
	...
	function agregar($data=array())
	{
		...
		$data['creado']=ahora();
		$this->ci->db->insert('cv_formacion',$data);
	...
	{
		return $this->ci->db->where('usuarios_ID',$this->id)->where('ID',$id)->get('cv_formacion')->row_array();
	...
	function borrar($id)
	{
		return $this->ci->db->where('usuarios_ID',$this->id)->where('ID',$id)->limit(1)->delete('cv_formacion');
